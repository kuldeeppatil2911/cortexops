...
const EMPTY_SERVICE = {
  name: '', description: '', environment: 'Production', endpoint: '', owner: '', status: 'Unknown', dependencies: ''
};
...
export default function ServiceRegistry({ services, onCreate, onUpdate, onDelete }) {
  ...
  const edit = (service) => { setEditingId(service._id); setForm({ ...service, dependencies: (service.dependencies || []).join(', ') }); };
  const checkService = async (service) => {
    try {
      const response = await fetch(`/api/services/${service._id}/health/check`, { method: 'POST' });
      if (!response.ok) throw new Error('Health check failed');
      const result = await response.json();
      setHealthResults((current) => ({ ...current, [service._id]: result }));
    } catch (err) { setError(err.message); }
  };

  return <div className="service-registry">
    <section className="registry-summary"><div><p className="kicker">SERVICE CATALOG</p><h3>Know what CortexOps is watching.</h3><p>Register monitored services, owners, endpoints, and dependencies. Health checks run automatically on the server.</p></div><div className="registry-count"><strong>{services.length}</strong><span>registered services</span></div></section>
